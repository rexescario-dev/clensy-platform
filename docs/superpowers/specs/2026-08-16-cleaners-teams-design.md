...
| **Revision note** | M3 round 1 (reviewer: project owner) returned the draft for contract-completeness fixes, not a redesign: `GetCleaner`/`GetTeam`/`ListTeamCleaners` promoted from "flagged addition" to formal application operations; same-state `assignCleanerToTeam` calls explicitly defined as a successful case; `Team.name` and `Cleaner.email` uniqueness decided (both unique, mirroring the `AdminUser.email` precedent) and `Cleaner.phone` explicitly decided non-unique; a GraphQL N+1 invariant, an explicit mutation/audit transaction-boundary statement, and a concurrent-assignment (last-write-wins) statement were added; a stray reference to a nonexistent `teamCleaners` GraphQL query was corrected to `team { cleaners }`. Round 2 confirmed the unconditional mutation/audit position (no diff-aware suppression) and tightened terminology: "no-op" replaced with "same-state" throughout normative text to avoid implying `updatedAt`/audit are skipped; `updatedAt`'s semantic (last successful mutation invocation, not last effective change) made explicit; §4.5's redundant issue-comparison sentence trimmed. |
| **M3 decision** | **Accepted** — 2026-08-16. No remaining design blocker. Ready for M4 Implementation Planning. |
...
- `createdAt: Date` (set once at creation; not client-settable), `updatedAt: Date` (not client-settable; set on every successful mutation *invocation* of this record — see the note below)

In this module, `updatedAt` records the time of the last successful mutation **operation**, not necessarily the time of the last effective field-value change: an `updateCleaner` call that supplies only already-current values, and a same-state `assignCleanerToTeam` call (§4.2), both bump `updatedAt`. This matches the existing `customers`/`admins` convention, applied consistently rather than reinterpreted per-field.
...
**On audit-event suppression for same-state mutations (resolved M3 round 2):** an earlier review round proposed that a same-team `assignCleanerToTeam` call, and more broadly any `updateCleaner` call whose provided values already match the cleaner's current state, should skip its audit event and `updatedAt` bump. This specification does **not** adopt that: `createCustomer`/`updateCustomer`/`AdminsService`'s mutations in this codebase already audit and timestamp unconditionally on every successful call, with no diff-aware suppression anywhere today, and this module stays consistent with that Accepted, cross-module contract rather than introducing a new, module-local audit philosophy. Applying the same unconditional rule to `assignCleanerToTeam`'s same-state case is what keeps this module internally consistent with itself (an assignment call and an update call would otherwise follow two different audit philosophies within one module). Confirmed by the project owner in the M3 round 2 review: every successful mutation invocation persists the entity, bumps `updatedAt`, and emits its audit event, even when the requested values equal the current ones. A cross-module, diff-aware audit-suppression mechanism remains a possible future change, but it is out of scope here and would need its own specification applied consistently across `customers`/`admins`/`cleaners` together, not as a `cleaners`-only exception.
...
`GetCleaner`, `GetTeam`, and `ListTeamCleaners` are formal application operations (§2, §4.2) because the web DoD requires cleaner/team detail views and the GraphQL presentation layer requires computed team membership — see §5 for why these were added beyond the issue's own Application bullet.
...
- `Team.name` MUST be non-empty at creation and **MUST be unique**, enforced the same way as `Cleaner.email` above (`ConflictException('Team name is already in use')`). Teams are identified by name in `apps/web`'s assignment dropdown (§4.6); allowing duplicates would make that selection ambiguous. There is no update path for `Team` in this slice (§2), so no post-creation revalidation applies to it. `CleanersService` and `TeamsService` each translate a Postgres `23505` unconditionally to their respective `ConflictException` message (matching `AdminsService`'s existing pattern, which does the same) — this stays unambiguous because each entity has exactly one unique column (`Cleaner.email`, `Team.name`); an implementer adding a second unique constraint to either entity in a later slice MUST inspect the driver error's constraint name rather than continuing to assume `23505` always means that one field.
...
- Same-state `assignCleanerToTeam` behavior (successful, unconditionally audited) and the GraphQL N+1 invariant (§4.5) are explicit.
- **Confirmed by owner (M3 round 2):** the module retains the existing unconditional mutation/audit convention for same-state assignments and no-effective-change updates — no diff-aware audit/`updatedAt` suppression (§4.4).
...
- A cross-module, diff-aware audit-suppression mechanism for same-state mutations — decided against for `cleaners` specifically (§4.4, §6); adopting it more broadly across `customers`/`admins` as well is a separate, cross-module specification if ever pursued.
