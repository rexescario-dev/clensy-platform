...
| **Constrained by** | [Multi-Tenant Architecture](2026-09-23-multi-tenant-architecture-design.md) (Accepted 2026-09-23): `OWNER` is replaced by `TENANT_OWNER` / `SUPER_ADMIN` (§4.3); the principal and `currentAdmin` gain `scope`/`tenantId` (§4.1); staff lifecycle and the last-owner rule are per tenant (§4.3). Implemented in [#68](https://github.com/rexescario-dev/clensy-platform/issues/68). |
