...
- **`issueDate`** — a single timestamp captured once at generation start and used consistently for the invoice's "as-of" moment and for the invoice-number year. Whether that year is derived in UTC or in Philippines business-local time is **deferred to M3** (§4.5, §6) — around local midnight on 1 January the two disagree.
...
- **How `issueYear` is derived from `issueDate` — UTC vs. Philippines business-local — is deferred to M3** (§3, §6). The business operates in one locale; around local midnight on 1 January `getUTCFullYear()` and a local-time year disagree, and an invoice-numbering year is a genuine domain choice, not an implementation detail. The function takes the year as an argument so the caller's timezone decision is explicit and testable.
...
- The invoice-numbering year's timezone basis (UTC vs. Philippines-local) is called out as an explicit M3 decision, not silently chosen (§3, §4.5).
...
- **Unit — `formatInvoiceNumber`:** `INV-2026-000042` for `(42, 2026)`; 6-digit zero-pad; `>999999` renders more digits without wrapping; `INV-2027-000101` for `(101, 2027)` (sequence value not reset per year). The caller's derivation of `issueYear` from `issueDate` (UTC vs. local) is an M3 decision and tested at that boundary once settled.
- **Unit — `InvoicesService.generateFromOrder`:**
  ...
  - **Eligibility** — rejects an order with `totalMinorUnits === null` (`BadRequestException`); rejects a `CANCELLED` / `REJECTED` order; rejects an order with no lines; (M3-pending: `COMPLETED` / `REFUNDED`).
...
- The Design Review decision itself (M3) — this document stops at **Draft** and does not self-Accept. Two bounded questions are explicitly left for M3: (a) whether `COMPLETED` / `REFUNDED` orders are also excluded from invoice generation (§4.3, §6); (b) whether the invoice-number year is derived in UTC or Philippines business-local time (§3, §4.5, §6).
