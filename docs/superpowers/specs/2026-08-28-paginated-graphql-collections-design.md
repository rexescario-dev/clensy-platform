...
| **Followed by (informative)** | After M3 Accept: M4 implementation planning for [#33](https://github.com/rexescario-dev/clensy-platform/issues/33) (module order belongs there). Quality (#7), Operations Dashboard (#8), and Payments (#10) remain independent product slices; when those specs exist they **MUST** inherit this RFC’s **platform** collection invariant. Admin GraphQL (`admins`) is a **legacy deferred exception** until its own migration spec. |
...
| **Revision note** | Third draft, then M3. Not a redesign. Round-1 return: get-by-id vs ReadResolver `one`. Round-2 human review: unproven 9.5.0 mechanics must not be locked as architecture. Third draft: inverse metadata preferred/M4-proven; O(1) nested loading; unique sort tie-breaker; `enableTotalCount` on root and nested options; clamp `limit > 100`; server-enforced `customerProperties` scope; allowlisted filters; no alternate unpaginated array; “introduced” includes future nested fields. |
| **M3 decision** | **Accepted** — 2026-08-28. File verified against the third-draft amendment list. No remaining design blocker. GraphQL contract and boundaries are locked; 9.5.0 realization is M4 proof (`property.bookings` is the cross-module proving nest). Ready for M4 Implementation Planning. |
...
- Central paging policy: **default 20**, **maximum 100**, defined once at platform level and enforced server-side (`maxResultsSize` or equivalent). Not independently per module; not a client convention.
- Connection shape: 9.5.0 OffsetConnection (`nodes`, `pageInfo`) **plus** `totalCount` on every root and nested connection, enabled via the applicable 9.5.0 option (not an OffsetConnection intrinsic; not a single global switch).
...
- Unidirectional-inverse TypeORM collection metadata as the **preferred** persistence representation for nested connections in §4.2, subject to the M4 9.5.0 proof (see §4.3). Owning-module-only `forFeature` / QueryService registration remains a hard gate (#29 §4.1 / §4.9#10).
...
- **Connection** — this RFC’s required collection return type: 9.5.0 OffsetConnection (`nodes`, `pageInfo`) **plus** `totalCount` on every root and nested connection (see §4.1). `totalCount` is not intrinsic to OffsetConnection; this RFC requires it to be enabled on the applicable 9.5.0 connection/query option.
...
- **Platform paging policy** — the single pair (default limit **20**, max limit **100**) that every in-scope connection uses, enforced server-side.
...
**Connection shape required by this RFC:** the standard 9.5.0 OffsetConnection shape (`nodes`, `pageInfo`) **plus** `totalCount`. `totalCount` is **not** intrinsic to OffsetConnection. Every in-scope **root and nested** connection MUST expose `totalCount`. The implementation MUST enable it with the applicable 9.5.0 option (`enableTotalCount` on `@QueryOptions` for root many-queries, and on `@OffsetConnection` / `@FilterableOffsetConnection` for nested relations, or equivalent) — not a custom count field, and not a single nonexistent global switch.
...
- `totalCount` is for DataTable-style “showing X of Y” / page-count UX without a second query. `hasNextPage` alone is enough for next/previous. It MUST NOT be computed unless the operation selects it.
- Omitted `paging` uses **limit 20, offset 0**.
- A requested `limit` greater than **100** is **clamped to 100** (server-side `maxResultsSize` or equivalent). It MUST NOT return more than 100 nodes. This RFC does **not** require a custom GraphQL error for oversized `limit`. Implementations MUST NOT set max size to unlimited (`-1`). The clamp MUST be uniform across modules.
...
**GraphQL contract (normative):** the nested fields in §4.2 (`customer.properties`, `team.cleaners`, `checklist.items`, `property.bookings`) MUST be offset connections with the same paging policy as root collections.

**Persistence representation (preferred, subject to proof):**

> The selected implementation MAY use TypeORM inverse collection metadata to represent ORM-backed nested connections. For this RFC’s inventory, inverse metadata is the **preferred** persistence representation where the relation is already represented by an FK. M4 MUST prove that this representation works with owning-module QueryService boundaries in 9.5.0.
...
**Loading (outcome, not library internals):** collections MUST NOT be eagerly loaded. Nested connection resolution MUST use a **constant-query** strategy such that SQL/query count is **O(1) with respect to the number of parent nodes**. The implementation MUST NOT issue one child query per parent node. How 9.5.0 achieves that (batch, grouped query, QueryService, or other) is M4, provided the O(1) bar holds (same evidence bar as #29 §4.8: SQL/query count, not spies on deleted loaders).
...
**M4 validation gate (architectural, not a task list):** M4 MUST prove the exact 9.5.0 nested-connection mechanism by executing a nested connection query **across real module boundaries** (at least `property.bookings`, the only cross-module nest in the table). TypeORM inverse metadata alone MUST NOT be assumed sufficient. The documented nestjs-query relation path uses a QueryService for relation resolution; whether owning-module export + import satisfies that without foreign `forFeature` is a **proof**, not a claim in this RFC. If the preferred inverse mapping cannot satisfy the hard gate, return to M2/M3 — do not silently mutate module boundaries.
...
3. **MUST** use default limit 20 and max 100 from one platform source, enforced server-side (`maxResultsSize` or equivalent); `limit > 100` **clamps** to 100; **MUST NOT** set unlimited max.
...
7. **MUST NOT** register foreign QueryServices/`forFeature` solely for DI; violation is a spec-return, not a workaround. Nested connections MUST be proven against 9.5.0 across module boundaries before claiming the hard gate is satisfied.
...
9. **MUST** keep nested collection SQL/query count **O(1) in parent N**; **MUST NOT** issue one child query per parent.
...
- **Why inverse collections (preferred).** Nested GraphQL connections for FK-backed relations are most honestly represented with ORM collection metadata (same N+1 lesson as #29’s many-to-ones). That representation is **preferred and subject to M4 proof**, not a claim that nestjs-query requires `@OneToMany` before a QueryService can resolve a relation. Restricting the table avoids a bidirectional graph for unused paths (`customer.bookings`).
- **Why 20 / 100.** Human-approved. The platform overrides library defaults; those default numbers are not this contract. Enforcement is 9.5.0 `maxResultsSize` (or equivalent server-side cap), not documentation. Oversized `limit` **clamps** so modules cannot diverge between silent cap and custom errors.
- **Why `totalCount`.** Pagination itself only needs `hasNextPage`. List UIs need DataTable-style “showing X of Y” / page-count without a second query. Enable it on **both** root `@QueryOptions` and nested `@OffsetConnection` (or equivalent) — there is no single global switch.
...
- OffsetPaging + 20/100 from one platform source, **enforced server-side**; `limit > 100` **clamps**; cursor and `PagingStrategies.NONE` stay out.
- `totalCount` is required on **root and nested** connections, enabled via the applicable 9.5.0 option (`enableTotalCount` or equivalent), not a custom field and not a single global switch.
...
- Nested GraphQL connections are the contract; inverse TypeORM metadata is **preferred**, subject to M4 proof; `forFeature` hard gate is unchanged; O(1) query count is the loading invariant (not “MUST batch”).
...
| Item | Status |
| --- | --- |
| 9.5.0 API for **one** platform default/max (module `forRoot` vs shared constant referenced by `@QueryOptions`) | **M4**. Product: default 20, max 100, **clamp** oversize `limit`. Enforcement MUST be `maxResultsSize` or equivalent server-side; scattering different numbers is a defect. |
| Nested connection QueryService wiring across module boundaries | **M4 proof** (§4.3). Execute a real nested connection (at least `property.bookings`). Inverse TypeORM metadata is **preferred**, not assumed sufficient. Foreign `forFeature` to “make DI work” is a spec-return. |
...
| `enableTotalCount` on root vs nested decorators | **Decided:** enable on both applicable 9.5.0 options. Not a single global switch. Count only when selected. |
