...
| `bookings` | Booking | "What a customer actually booked." Holds real `customerId`/`propertyId`/`serviceId` relations (see the [Bookings spec](2026-08-22-bookings-design.md)) rather than free-text customer/service strings. Captures a pricing snapshot at booking time rather than a live reference to current catalog pricing — a later price change in `catalog` must not retroactively alter what a historical booking appears to have cost. REST surface, tests, and status model preserved through that migration — it was a migration, not a rewrite. |
...
Booking GraphQL-read persistence metadata is amended by [nestjs-query GraphQL Reads](2026-08-28-nestjs-query-graphql-reads-design.md): `BookingEntity` may declare unidirectional TypeORM relations so nestjs-query can batch GraphQL relation loads. Application-layer writes, dashboard reads, and domain objects remain bound by the rule above.

## 3. Web Architecture & Testing

**apps/web** (Next.js, App Router): `/login` is the sole public route; every authenticated route lives under `/app/*`, mirroring module boundaries — `/app/customers`, `/app/cleaners` + `/app/cleaners/teams`, `/app/catalog` + `/app/catalog/add-ons`, `/app/admin` (staff/roles) are shipped (M5 — Dashboard UX Foundation), with `/app/bookings`, `/app/jobs`, `/app/quality` to follow as those modules ship. `/app` itself redirects to the first available module (currently `/app/customers`) until it becomes the Operations Dashboard (M9). Middleware at the `/app/*` boundary is a cookie-presence UX gate only, not the security boundary; authorization semantics mirror the six-role matrix defined by `platform/auth`, but `apps/web` does not depend on `apps/api`'s Nest infrastructure directly. The API (`platform/auth`) remains the authoritative enforcement point — frontend route protection is UX/security layering, not the source of authorization truth. Unauthorized requests redirect to `/login`. Every screen goes through `packages/client` for data and `packages/ui` for components — no screen hand-rolls its own query string or button.
