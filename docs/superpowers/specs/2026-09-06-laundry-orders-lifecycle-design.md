...
2. Validate: exactly one `baseServiceId` (with an optional `baseQuantity`); add-on inputs each have an `addOnId` and an optional `quantity`; every supplied quantity is an integer `≥ 1` (default 1); `order.weightGrams` is set. A supplied quantity is used only when that line's resolved `unit = PER_ITEM`; for `PER_KG` the line quantity is `order.weightGrams` and for `FLAT`/`PER_SERVICE` it is `1`, regardless of any supplied value.
...
- `quantity` fed in — and stored on the snapshot verbatim — is: `order.weightGrams` when `unit = PER_KG` (an **integer count of grams**, never kilograms); the caller's per-line item count when `unit = PER_ITEM`; `1` when `unit ∈ {FLAT, PER_SERVICE}` (any caller-supplied quantity ignored).
- `raw`:
