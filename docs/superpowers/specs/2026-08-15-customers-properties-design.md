...
| **Extended by** | [Multi-Tenant Architecture](2026-09-23-multi-tenant-architecture-design.md) §4.4–§4.5 (shipped in [#82](https://github.com/rexescario-dev/clensy-platform/issues/82)). It makes Customer and Property tenant-owned, requires Property → Customer to stay within one tenant, and adds per-tenant, case-insensitive customer email uniqueness. That uniqueness **supersedes** this spec's §2 non-goal "deduplication or uniqueness enforcement on customer email". |
