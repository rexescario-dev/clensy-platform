...
| **Revision note** | M5 round 1 (reviewer: project owner) returned the draft for one required correction and five tightenings. Required: Task 1's migration-backfill test claim was internally inconsistent (claiming the backfill was verified while also explicitly disclaiming migration execution) — resolved by replaying the migration's exact backfill SQL against a throwaway temp table seeded with legacy-shaped data (verified first that no existing test infrastructure could do this more cheaply, and that toggling the shared `pricing_rule_entity` table's schema via `down()`/`up()` mid-suite was a real operational risk, not merely inconvenient, before choosing the temp-table approach). Tightenings applied: the close-and-read/forward-only rollback dependency was made explicit; the ternary target derivation was replaced with one discriminated branch (which, while fixing this, was caught mid-edit accidentally weakening "both provided" into a silent `serviceId` preference — corrected before it propagated, since that would have violated spec §4.7); a required `migration:generate` diff-and-verify step against existing `PricingRule` indexes was added; "tests unmodified" was given a precise definition; the concurrency test's assertions were strengthened to check the full resulting chain; and a defensive `ORDER BY` on `resolveEffectivePricing` was considered and declined with reasoning (the Accepted spec already guarantees at-most-one-match by construction). M5 round 2 requested two further minor tightenings, applied: the temp-table replay's wording was made precise that it verifies the backfill SQL's own semantics (partitioning, tie-breaking) via SQL shared verbatim with the migration, not that the migration file itself was executed; and the replay fixture was expanded from a single tied-timestamp pair to three services (multi-row partitioning, a single-row partition, and the tied pair) so partitioning and tie-breaking are both proven, not only the tie case. The `addOnId`/`serviceId` entity index names were also made explicit to match the migration's actual index names exactly. |
| **M5 decision** | **Accepted** — 2026-09-06. All items from both review rounds verified present and consistent, not merely asserted. No remaining design or planning blocker. Ready for M6 Implementation. |
...
- **The close-and-read step (spec §4.4 step 3) is a `QueryBuilder` `UPDATE ... RETURNING`, not `manager.update()`**, because `manager.update()` cannot return the row it modified and this step's forward-only check needs the prior open interval's `effectiveFrom` in the same statement. The target is resolved via one explicit discriminated-branching block, which also performs the mutual-exclusivity check as the same condition (below) — not a ternary re-derived later from separately-validated fields. One `{ column, id }` pair is computed once and threaded through every subsequent step, so there is exactly one place in the method that decides which column this call targets:
  ```ts
  const hasServiceId = command.serviceId !== undefined;
  const hasAddOnId = command.addOnId !== undefined;
  if (hasServiceId === hasAddOnId) {
    // True when both are provided, or neither is — both are rejections.
    throw new BadRequestException('Exactly one of serviceId or addOnId is required');
  }
  const target: { column: 'serviceId' | 'addOnId'; id: string } = hasServiceId
  ...
  const closeResult = await manager
    ...
    .where(`"${target.column}" = :targetId AND "effectiveTo" IS NULL`, { targetId: target.id })
  ...
  ```
  `target.column` is one of exactly two literal string values fixed by the `if`/`else if`/`else` branching above, never user-supplied text — the interpolation is safe from injection because the branching, not caller input, decides which of the two compile-time literals appears; `target.id` itself is always passed as a bound parameter, never interpolated. **The close-and-read `UPDATE` and the forward-only validation execute inside the same database transaction as every other step of this method** (the surrounding `this.dataSource.transaction(...)` call, unchanged from today): if `priorEffectiveFrom`'s check throws, the transaction rolls back the close it just performed along with everything else in the call, leaving the open interval exactly as it was before this call started — the correctness of "a rejected call never partially applies its close" rests on this transactional boundary, not on the validation ordering alone, and Task 2's tests (below) assert the rolled-back state directly, not merely that an exception was thrown.
...
- **Mutual exclusivity is enforced by the discriminated target-resolution code itself (above), not a separate earlier check duplicating the same condition** — `hasServiceId === hasAddOnId` is `true` for exactly the two rejection cases (both provided, or neither), computed with `!==` `undefined` (not truthiness, so an explicitly-passed empty string is still treated as "provided," not silently ignored) and both cases throw the same `BadRequestException`, matching spec §4.7's explicit requirement that the application layer reject "both provided" rather than silently prefer one target over the other. This is deliberately not a `class-validator` cross-field decorator on `CreatePricingRuleInput` — this codebase has no existing precedent for one, and the spec itself attributes this check to the application layer (§4.7), not the GraphQL input layer.
...
- **`resolveEffectivePricing`'s query is a `QueryBuilder`, not `findOneBy`**, because the interval predicate (`effectiveFrom <= asOf AND (effectiveTo IS NULL OR effectiveTo > asOf)`) is not expressible as a plain `FindOptionsWhere` object:
  ...
  ```
  **Considered and declined (M5 round 1): a defensive `ORDER BY "effectiveFrom" DESC` before `.getOne()`**, to silently pick "the latest" if the invariant were ever violated by a bug elsewhere. Not adopted — the Accepted spec states this exactly (§4.3: "at most one row can ever satisfy this predicate for a given target and `asOf`, by construction... this method does not need to guard against or arbitrate multiple matches"), and adding silent arbitration here would mask a genuine data-integrity bug (two overlapping open intervals existing at all) behind a plausible-looking result instead of surfacing it. If that invariant is ever violated in practice, this method returning a nondeterministic-looking result (whichever row Postgres happens to return first) is a feature, not a gap — it makes a `LIMIT`-free query behave differently across runs, which is a more honest signal that something upstream is broken than a deterministic-but-wrong answer would be.
...
- **Verify `migration:generate` proposes no change to any pre-existing `PricingRule` index or constraint.** This migration's schema changes (`serviceId` becoming nullable, the new `unit`/`effectiveFrom`/`effectiveTo`/`minimumChargeMinorUnits`/`addOnId` columns) touch only columns, not `serviceId`'s existing plain `@Index()` (`IDX_c22f021ac1046f25883817f8f9`, from the original `AddPricingRule` migration) or `uq_pricing_rule_active_service` — a standard Postgres btree index does not need to be dropped and recreated when its underlying column's nullability changes, so no diff is structurally expected here. This plan does not merely assume that: after running `pnpm migration:generate`, the generated file's statements MUST be diffed line-by-line against §3's exact statement list below before hand-editing — any statement touching `IDX_c22f021ac1046f25883817f8f9` or `uq_pricing_rule_active_service` is spurious generator drift (the same category `AddPricingRule`'s own migration comment already documents for `fk_property_customer`/`fk_cleaner_team`) and MUST be deleted from the generated file, not executed. This is a required verification step for Task 1, not an assumption carried into it.
...
- **Backfill correctness is verified by executing the migration's exact backfill SQL against legacy-shaped seed data in an isolated fixture, not merely inferred from post-migration behavior.** Rows created through the already-migrated entity (in the Catalog-plan-style tests) prove the new columns' defaults and the entity mapping — they do **not** prove the migration's `LEAD(..., "id")` backfill was computed correctly against data that existed *before* this migration ran, because no such pre-existing row can be produced by creating one after the fact. Toggling the shared `pricing_rule_entity` table's schema mid-suite (running this migration's own `down()` then `up()` against it) was considered and rejected: this file's tests share that one table under the existing advisory lock, and a failure partway through such a toggle would leave the schema in a broken state for every other test in the suite — a real operational risk to the shared test database, not merely inconvenience, and `apps/api/test` has no existing precedent for it (verified: no test in this codebase currently imports a migration class or calls `.up()`/`.down()` directly).
  Instead, Task 1 seeds a **throwaway temporary table** — `CREATE TEMP TABLE pricing_rule_backfill_fixture ("id" uuid, "serviceId" uuid, "createdAt" timestamptz)`, scoped to the test's own connection/session and dropped automatically at session end, touching no shared state — with rows resembling real legacy data across multiple services (§8's Task 1 test list gives the exact fixture shape), then runs the *exact* `effectiveFrom`/`effectiveTo` backfill statements from §3's migration list (copied verbatim, not re-derived) against that fixture table, and asserts the resulting `effectiveFrom`/`effectiveTo` values against hand-computed expected values. `dataSource.createQueryRunner()` is already an established primitive in this test suite (the existing concurrency-test warm-up pattern) — this reuses it for a new, self-contained purpose rather than inventing a new connection mechanism. **This test verifies the backfill SQL's semantics — that the `LEAD(..., "id")` expression, run against legacy-shaped data, produces the correct partitioning and tie-breaking — not that the real migration file was executed.** The actual migration (§3's numbered statement list) remains the sole thing responsible for applying that same SQL to the real `pricing_rule_entity` table when it runs in any real environment; this test and that migration file share the identical SQL text by construction (copied verbatim, not re-derived), which is what makes a pass here meaningful evidence about the migration rather than evidence about an unrelated query.
...
Three test levels, matching every prior slice in this codebase. **Every "re-run unmodified" claim below means exactly this, not "the file is untouched":** the existing test's assertions remain source-identical and must continue to pass; no existing assertion is weakened, relaxed, or deleted to accommodate this ticket's schema change. The test *files* are legitimately extended (new `describe` blocks, extended `beforeEach` truncation lists) — what must not change is any pre-existing `it(...)`'s body or expected value.
...
- `apps/api/src/modules/catalog/infrastructure/persistence/pricing-rule.entity.ts`:
  - `serviceId`: `@Column({ type: 'uuid', nullable: true }) @Index('IDX_c22f021ac1046f25883817f8f9')` (nullability added; the index name is now given explicitly, matching the name TypeORM already auto-generated for it in the original `AddPricingRule` migration — a pure clarity change with no schema effect, since the DB index already has this exact name).
  - `addOnId`: `@Column({ type: 'uuid', nullable: true }) @Index('IDX_pricing_rule_addon_id')` — plain column, no `@ManyToOne`, same no-relation-decorator precedent as `serviceId` (spec §4.1's inherited reasoning). The name is given explicitly to match the migration's `CREATE INDEX "IDX_pricing_rule_addon_id"` statement (§3) exactly — letting TypeORM auto-generate a name here would produce a hash-based name that doesn't match what the migration actually creates, which the `migration:generate` diff check (§3) would otherwise need to reconcile by inspection every time instead of by direct comparison.
...
- **Column defaults for post-migration inserts (proves entity mapping, not backfill):** every `PricingRule` row created via `createPricingRule` in this same test, using **only** today's call shape (`serviceId` + `priceMinorUnits`, no new fields), has `unit: PricingUnit.PER_SERVICE`, `effectiveFrom` within a tight time-window of "now," `minimumChargeMinorUnits: null`, `addOnId: null`. This test says nothing about pre-existing data — it only proves the entity/column defaults are correct for rows created after the migration, which is a different claim from backfill correctness (§3).
- **Backfill replay against legacy-shaped data (verifies the backfill SQL's own semantics — partitioning and tie-breaking — against data shaped like it predates this migration; per §3):** create a `TEMP TABLE` (session-scoped, auto-dropped, no shared-state risk) shaped like the pre-migration `pricing_rule_entity` (`id`, `serviceId`, `createdAt` only), seed it via `manager.query()` with this exact fixture:
  ```text
  Service A: R1 createdAt=T1, R2 createdAt=T2, R3 createdAt=T3   (T1 < T2 < T3)
  Service B: R4 createdAt=T4
  Service C: R5 createdAt=T5, R6 createdAt=T5                    (tied timestamp)
  ```
  then execute this migration's exact `effectiveFrom`/`effectiveTo` backfill `UPDATE` statements (copied verbatim from §3, targeting the temp table) against it, and assert: `A` chains `R1.effectiveTo=T2 → R2.effectiveTo=T3 → R3.effectiveTo=NULL` (proves multi-row partitioning within one service, not just a two-row case); `B`'s single row gets `effectiveTo=NULL` (proves a single-row partition doesn't get spuriously chained to a *different* service's rows — the `PARTITION BY "serviceId"` clause, not just `LEAD` in isolation); `C`'s tied pair deterministically orders the lower `id` as superseded (`effectiveTo=T5`) with the higher `id` open (`effectiveTo=NULL`). One fixture, one test, proving partitioning and tie-breaking together rather than only the tie case in isolation.
- **Backfill tie-breaker determinism, repeated:** re-run the assertion against Service C's tied pair (from the same fixture above) 3× against freshly-reseeded temp tables, to rule out order-dependent flakiness in the `LEAD(..., "id")` expression itself.
- The `CHECK (num_nonnulls("serviceId", "addOnId") = 1)` constraint: a direct `manager.query()` `INSERT` against the **real** (post-migration) `pricing_rule_entity` table with both `serviceId` and `addOnId` set (or both `NULL`) throws a Postgres constraint-violation error — proves the database-level invariant holds independently of the application-layer check that will be added in Task 2 (this task adds no application code that could intercept it first).
...
- Regression: every existing Task-1/2/3-level test from the Catalog plan for `Service`/`AddOn`/`PricingRule` continues to pass with its assertions unmodified (§7's definition) — this task changes no application code, only schema, so a pass here is evidence the migration's *column-level* additivity holds; it is not, by itself, proof that the migration's *backfill* is correct against real pre-existing data — that is the dedicated backfill tests above (schema-shaped introspection, tie-breaker determinism, and the pre-migration-data replay in §3/§8's temp-table test), not this regression check.
...
- **Concurrency race (both target kinds, matching the Catalog plan's existing concurrency-test precedent):** `Promise.allSettled([svc.createPricingRule(cmdA), svc.createPricingRule(cmdB)])` for the same `serviceId` (and, as a second case, the same `addOnId`) with two different valid future `effectiveFrom` values. Beyond "exactly one fulfilled, one rejected with `ConflictException`" (necessary but not sufficient — a test could pass that shape merely because an unrelated later operation failed), assert the full resulting chain: exactly two rows total exist for that target after the race (the rejected call leaves no extra row — the row count, not just the `effectiveTo IS NULL` count, is checked); exactly one row has `effectiveTo IS NULL`, and it is specifically the *fulfilled* call's own row (identified by the `PricingRule` returned from the fulfilled promise, not merely "some row"); the predecessor row's `effectiveTo` equals the fulfilled call's `effectiveFrom` exactly (the close-and-insert pairing is internally consistent, not just "a close happened"); and, for the `serviceId` case specifically, the legacy `active` flag is consistent with whichever call actually won — `active: true` on the fulfilled row only if its `effectiveFrom <= operationNow` (this test uses immediate `effectiveFrom` values for the `serviceId` case specifically so this assertion is exercised, not skipped).
