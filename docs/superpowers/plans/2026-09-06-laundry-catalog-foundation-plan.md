...
- **Backfill correctness is verified by actually executing the migration's own backfill SQL against legacy-shaped seed data, not merely inferred from post-migration behavior.** Rows created through the already-migrated entity (in the Catalog-plan-style tests) prove the new columns' defaults and the entity mapping — they do **not** prove the migration's `LEAD(..., "id")` backfill was computed correctly against data that existed *before* this migration ran, because no such pre-existing row can be produced by creating one after the fact. Toggling the shared `pricing_rule_entity` table's schema mid-suite (running this migration's own `down()` then `up()` against it) was considered and rejected: this file's tests share that one table under the existing advisory lock, and a failure partway through such a toggle would leave the schema in a broken state for every other test in the suite — a real operational risk to the shared test database, not merely inconvenience, and `apps/api/test` has no existing precedent for it (verified: no test in this codebase currently imports a migration class or calls `.up()`/`.down()` directly).
  Instead, Task 1 seeds a **throwaway temporary table** — `CREATE TEMP TABLE pricing_rule_backfill_fixture ("id" uuid, "serviceId" uuid, "createdAt" timestamptz)`, scoped to the test's own connection/session and dropped automatically at session end, touching no shared state — with rows resembling real legacy data across two different services (including one intentionally-tied `createdAt` pair), then runs the *exact* `effectiveFrom`/`effectiveTo` backfill statements from §3's migration list (copied verbatim, not re-derived) against that fixture table, and asserts the resulting `effectiveFrom`/`effectiveTo` values against hand-computed expected values for the whole multi-row, multi-service scenario. `dataSource.createQueryRunner()` is already an established primitive in this test suite (the existing concurrency-test warm-up pattern) — this reuses it for a new, self-contained purpose rather than inventing a new connection mechanism. This is genuine execution of the migration's backfill logic against pre-existing-shaped data, with zero risk to any other test in the file, and it directly answers "does the backfill produce the right answer," not merely "is the expression well-formed" (which the standalone tie-breaker-determinism test still covers separately, per §7/§8, since it isolates that one specific property).
...
- **Backfill replay against legacy-shaped data (proves the actual backfill computation, per §3):** create a `TEMP TABLE` (session-scoped, auto-dropped, no shared-state risk) shaped like the pre-migration `pricing_rule_entity` (`id`, `serviceId`, `createdAt` only), seed it via `manager.query()` with rows for at least two different services, including one intentional tied-`createdAt` pair for one service, then execute this migration's exact `effectiveFrom`/`effectiveTo` backfill `UPDATE` statements (copied verbatim from §3, targeting the temp table) against it, and assert the resulting `effectiveFrom`/`effectiveTo` on every seeded row matches hand-computed expected values — including that the tied-timestamp pair's lower `id` is treated as superseded first. This is the test that actually answers "does the backfill produce correct history for data that existed before this migration," not an inference from post-migration behavior.
- **Backfill tie-breaker determinism, isolated:** the same tied-`createdAt` scenario as above, run 3× against a freshly-reseeded temp table each time, to rule out order-dependent flakiness in the `LEAD(..., "id")` expression itself, independent of the fuller multi-service scenario above.
