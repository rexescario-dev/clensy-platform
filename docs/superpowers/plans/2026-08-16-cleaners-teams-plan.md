...
- **Contract: `updateCleaner` MUST only ever assign fields represented by `UpdateCleanerCommand` — never `teamId`, `id`, or either timestamp — regardless of the exact code used to apply them.** The concrete mechanism this plan specifies is object spread at the resolver (`{ ...input, actorId }`, never manual per-field listing) feeding `Object.assign(entity, command)` in the service, because manual field-by-field listing of a fully-optional `PartialType` input would materialize `undefined` for every omitted key, and `Object.assign` would then use those `undefined`s to overwrite the entity's existing values — silently breaking the omitted-fields-retain-current-value guarantee. An implementer may use an equivalent mechanism that satisfies the same contract; the object-spread approach is this plan's recommendation because it is what `CustomersService`/`PropertiesService` already do, not because no other correct implementation exists.
...
- `apps/api/test/cleaners-teams.e2e-spec.ts` — reuses `apps/api/test/helpers/seed-owner.ts` to seed and log in as the fixture Owner; unique-per-run data and id-scoped assertions throughout (§7 — no truncation, no lock needed):
  ...
  5. **Batching/query-count proof, distinct from step 4** (§7): before issuing the request, `jest.spyOn(app.get(TeamsService), 'getTeamsByIds')` and `jest.spyOn(app.get(CleanersService), 'listCleanersByTeamIds')`. Re-issue a `cleaners { team { name } }` request over the same ≥3-cleaner/≥2-team fixture from step 4 → assert `getTeamsByIds` was called **exactly once**, with an argument array containing every distinct non-null `teamId` referenced by the result set. Separately, issue a `teams { cleaners { id } }` request over the same fixture → assert `listCleanersByTeamIds` was called **exactly once**, with an argument array containing every team id in the result set. This is the test that actually fails if a resolver regresses to one `getTeam`/`listTeamCleaners` call per parent row — step 4 alone would still pass in that regression, since both a batched and an N+1 implementation return identical data.
  6. Use the existing Owner-only `createAdmin` mutation to create a Scheduler, a Customer Support, a Finance, and an Analyst admin **within this suite** for the RBAC checks below — this suite does not depend on fixtures created by, or the execution order of, the Customers & Properties e2e suite or any other file.
  7. Log in as the Scheduler → `cleaners` query succeeds (view-allowed) → `createCleaner` denied (write-restricted).
  8. Log in as the Analyst → `cleaners` query succeeds (view-allowed).
  9. Log in as the Customer Support admin → `cleaners` query denied (no view access per §4.3's matrix) → `createCleaner` denied.
  10. Log in as the Finance admin → `cleaners` query denied → `createCleaner` denied.
  11. `assignCleanerToTeam` with a nonexistent `teamId` (as Owner) → rejected with a not-found error, the cleaner's `team` is unchanged on re-fetch.
  12. `createTeam` with a `name` matching an already-existing team → rejected with a conflict error, no duplicate persisted.
