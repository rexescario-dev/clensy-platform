...
| **Revision note** | Three M5 rounds (reviewer: project owner), all targeted corrections, no redesign. Round 1: explicit `CleanerType`/`teamId`-exposure decision, a real query-count test for the N+1 invariant, suite-local RBAC fixtures, simplified loader code, explicit failed-mutation-audit and omitted-vs-null semantics. Round 2: fixed two plan-internal bugs — `updateCleaner`'s `Object.assign(entity, command)` would have leaked `actorId` onto the entity, and `toCleanerType()`'s `Omit<CleanerType, 'team'>` description would have stripped `teamId` before `Cleaner.team`'s resolver could read it — plus a stale duplicate copy of the loader code, a stricter batching assertion, and a request-isolation e2e test. Round 3: `assignCleanerToTeam` switched from `Object.assign` + `manager.save()` to `manager.update()` with an explicit `updatedAt: new Date()`, because `save()`'s change-diffing risked a no-op `UPDATE` for same-state assignments, silently violating the spec's unconditional-`updatedAt` requirement; the corresponding test assertion is now a required strict advance, not a soft check. Self-review after round 3 found the identical risk applied to `updateCleaner` (spec §4.2 makes the same unconditional-`updatedAt`/audit guarantee for a no-effective-change update) but the round-3 fix had only been applied to `assignCleanerToTeam` — `updateCleaner` switched to the same `manager.update()` mechanism for consistency, with a matching real-Postgres test added. `dataloader` approved as this slice's batching dependency. |
| **M5 decision** | **Accepted** — 2026-08-16. `dataloader` approved. No remaining blockers. Ready for M6 Implementation. |
...
- **Contract: `updateCleaner` MUST only ever persist fields represented by `UpdateCleanerCommand`'s non-`actorId` fields — never `actorId` itself, `teamId`, `id`, or either timestamp except the explicit `updatedAt` bump — regardless of the exact code used to apply them, and MUST persist a real database `UPDATE` (with `updatedAt` advancing) even when every provided field already equals the cleaner's current value.** Two mechanisms are required together, for two different reasons: object spread at the resolver (`{ ...input, actorId }`, never manual per-field listing) — a fully-optional `PartialType` input listed field-by-field would materialize `undefined` for every omitted key, corrupting the omitted-fields-retain-current-value guarantee if it ever reached an `Object.assign`; and, in the service, destructuring `actorId` out of `command` (`const { actorId, ...changes } = command`) and passing only `changes` (plus an explicit `updatedAt: new Date()`) to `manager.update(CleanerEntity, { id }, { ...changes, updatedAt: new Date() })` — never `Object.assign(entity, command)` (would leak `actorId` onto the entity) and never `Object.assign(entity, changes)` followed by `manager.save(entity)` (would let TypeORM's change-diffing silently skip the `UPDATE`, and therefore the `updatedAt` bump, when `changes`' values already match the entity). An implementer may use an equivalent mechanism that satisfies the same contract; this plan's recommendation departs from `CustomersService`/`PropertiesService`'s plain `Object.assign` + `save()` specifically because those methods were never required to guarantee an `updatedAt` bump on a same-value call — `updateCleaner` is (spec §4.2).
- **`CleanersService`/`TeamsService` follow the `AdminsService`/`CustomersService` transaction/audit pattern exactly** — the transaction wrapper and the audit call within it, together:
  ...
  ```
  `runAuditInTransaction(manager, fn)` only stashes `manager` in `AsyncLocalStorage`; the `this.auditLogger.log(...)` call inside `fn` is the same `@Inject(AUDIT_LOGGER) auditLogger: AuditLogger` constructor-injected dependency `AdminsService`/`CustomersService` already use — nothing new. `assignCleanerToTeam` uses the same wrapper but **MUST NOT** rely on `manager.save(cleaner)` alone for the same-state case: TypeORM's `save()` diffs the in-memory entity against the currently-persisted row and omits unchanged columns from the generated `UPDATE`'s `SET` clause, so a same-state `teamId` (identical to the row's current value) risks producing an `UPDATE` that touches no columns at all — silently failing the §2 requirement that `updatedAt` bump even then. The mechanism this plan specifies instead: `manager.findOneBy(TeamEntity, { id: teamId })` (existence check) → `manager.findOneBy(CleanerEntity, { id: cleanerId })` (existence check) → `manager.update(CleanerEntity, { id: cleanerId }, { teamId, updatedAt: new Date() })`, an unconditional, diff-independent `UPDATE` that always sets both columns regardless of their current values → `manager.findOneByOrFail(CleanerEntity, { id: cleanerId })` to obtain the updated entity for the audit call and return value → audit `cleaner.assign_team`. Reads (`getCleaner`, `getTeam`, `listCleaners`, `listTeams`) use plain injected `Repository`s — no transaction needed.
...
| `apps/api/package.json` (add `dataloader`, approved — M5) | Every other dependency entry |
...
- `apps/api/src/modules/cleaners/application/services/cleaners.service.ts` — `CleanersService`:
  ...
  - `updateCleaner(id, command)`: same transaction/audit wrapper; `manager.findOneBy(CleanerEntity, { id })`, throws `NotFoundException('Cleaner ${id} not found')` if absent; `const { actorId, ...changes } = command;` — **never `Object.assign(entity, command)` directly**, since `command.actorId` is audit metadata, not a `Cleaner` field, and `CleanerEntity` has no `actorId` property to receive it (an M5 review caught this exact bug in this plan's own text: an earlier draft of this bullet described `Object.assign(entity, command)`, which would have assigned `actorId` onto the entity, contradicting §3's own contract). Validates against the **resulting** state without mutating the tracked entity yet: `this.assertValid({ ...entity, ...changes })`. Then persists via `manager.update(CleanerEntity, { id }, { ...changes, updatedAt: new Date() })` inside a `try/catch` translating `23505` to `ConflictException('Email is already in use')` — **not** `Object.assign(entity, changes)` followed by `manager.save(entity)` — for the same reason as `assignCleanerToTeam` below: spec §4.2 requires `updateCleaner` to bump `updatedAt` and audit unconditionally even when every provided field already equals the cleaner's current value, and `save()`'s change-diffing would risk a no-op `UPDATE` (and therefore no `updatedAt` bump) in exactly that case. Then `const updated = await manager.findOneByOrFail(CleanerEntity, { id })`; logs `cleaner.update` using the destructured `actorId` and `updated.id`; returns `updated`.
  - `assignCleanerToTeam(command)`: same transaction/audit wrapper; `manager.findOneBy(TeamEntity, { id: command.teamId })`, throws `NotFoundException('Team ${command.teamId} not found')` if absent; `manager.findOneBy(CleanerEntity, { id: command.cleanerId })`, throws `NotFoundException('Cleaner ${command.cleanerId} not found')` if absent; **`await manager.update(CleanerEntity, { id: command.cleanerId }, { teamId: command.teamId, updatedAt: new Date() })`** — not `Object.assign` + `manager.save()`, because `save()`'s change-diffing would risk a no-op `UPDATE` when `teamId` already equals its current value, which would silently violate the §2 requirement that `updatedAt` bump unconditionally; `manager.update()` issues a direct, diff-independent `UPDATE` that always sets both columns; then `const updated = await manager.findOneByOrFail(CleanerEntity, { id: command.cleanerId })` to obtain the fresh row; logs `cleaner.assign_team` with `entityType: 'cleaner'`, `entityId: updated.id`; returns `updated`.
  ...
  - `listCleanersByTeamIds(teamIds: string[])`: injected `Repository<CleanerEntity>.findBy({ teamId: In(teamIds) })` — bulk method for Task 3's loader; not exposed over GraphQL directly. **Exists alongside `listTeamCleaners`, not instead of it**: the Accepted spec (§4.2, §4.5) names `listTeamCleaners` explicitly as a required application-layer operation, while leaving the N+1 invariant's batching mechanism unspecified — this method is this plan's M4-only addition for that mechanism, not a replacement for spec-named surface.
...
- `updateCleaner`: a command with only `phone` set leaves `fullName`/`email`/`notes` unchanged in the re-read row; `notes: null` explicitly clears an existing value (proves explicit-null-vs-omitted against real persisted state, mirroring the Customers precedent). **Given two persisted cleaners A and B, updating B with B's own `email` set to A's `email` throws `ConflictException`, and B's row is unchanged on re-read** — the same `23505` → `ConflictException` translation `createCleaner` already proves, exercised through the `updateCleaner` code path specifically, since it's a separate `try/catch` around a separate `manager.update()` call (§3). **Calling `updateCleaner` with every field set to the cleaner's own current values (a no-effective-change update) still strictly advances `updatedAt`** (`toBeGreaterThan`, required assertion) **and still emits a `cleaner.update` audit event** — the concrete real-persistence proof that `manager.update()` (not `save()`) makes this guarantee hold for `updateCleaner` the same way it does for `assignCleanerToTeam`'s same-state case above.
- `assignCleanerToTeam`: given a persisted `Cleaner` and two persisted `Team`s, assigning to the first team sets `teamId` and audits `cleaner.assign_team`. Assigning to the same team again re-persists and audits a **second** `cleaner.assign_team` event, **and** `updatedAt` strictly advances (`expect(after.updatedAt.getTime()).toBeGreaterThan(before.updatedAt.getTime())`) — both are required assertions, not one primary and one secondary, since `manager.update()` (not `save()`, per §3's fix) always issues the `UPDATE` unconditionally regardless of whether `teamId`'s value actually changed, so there is no diffing-related reason `updatedAt` could fail to advance. Given a nonexistent `teamId`, throws `NotFoundException` and the cleaner's `teamId` is unchanged afterward.
...
**Files (new):**
- `apps/api/src/modules/cleaners/presentation/graphql/cleaner.type.ts` — `@ObjectType('Cleaner')` class declares `@Field()`s for exactly: `id` (`ID`), `fullName`, `phone`, `email`, `notes` (nullable), `team` (`TeamType`, nullable), `createdAt`, `updatedAt` — no `@Field()` for `teamId`, so GraphQL's schema/introspection never exposes it and no client can query it (§2's `CleanerType`-exposure decision). **`toCleanerType(cleaner: Cleaner): CleanerType` must preserve `teamId` on the returned object even though `CleanerType` has no `@Field()` for it** — GraphQL exposure is controlled by `@Field()` decorators, not by which properties exist on the runtime object, so `teamId` stays readable to `@ResolveField()` while remaining absent from the public schema. `Cleaner.team`'s `@ResolveField()` (below) is typed against `Pick<Cleaner, 'id' | 'teamId'>`, not `CleanerType`, so the persistence key it needs stays available internally without becoming part of the GraphQL contract.
...
- `apps/api/package.json` — add `dataloader` dependency (approved, §3)
...
- `CleanerResolver.team`: when `cleaner.teamId` is `null`, the method returns `null` synchronously and never calls `loaders.teamLoader.load(...)` (mocked `CleanerTeamLoaders`, asserting the mock's `load` was not called) — proves the short-circuit exists in code rather than relying on `DataLoader` to handle a `null`/`undefined` key gracefully.
...
- `apps/web/app/cleaners/tests/`: none — matches the Customers & Properties plan's Task 5 precedent: Task 6's e2e verifies the GraphQL API's behavior, not the Next.js pages; the manual acceptance checklist below is what verifies the web layer itself.
...
- `apps/api/test/cleaners-teams.e2e-spec.ts` — reuses `apps/api/test/helpers/seed-owner.ts` to seed and log in as the fixture Owner; unique-per-run data and id-scoped assertions throughout (§7 — no truncation, no lock needed):
  ...
  4. Fixture: Team A with 2 cleaners, Team B with 1 cleaner, Team C with 0 cleaners, plus 1 cleaner left unassigned. `cleaners { team { name } }` → asserts the correct `team`/`null` per row (functional correctness). `teams { cleaners { id } }` → asserts Team A returns its 2 cleaners, Team B its 1, Team C `[]` — the empty-team case specifically proves the grouping logic defaults correctly rather than merely omitting keys with no matches.
  5. **Batching/query-count proof, distinct from step 4** (§7): before issuing the requests, `jest.spyOn(app.get(TeamsService), 'getTeamsByIds')` and `jest.spyOn(app.get(CleanersService), 'listCleanersByTeamIds')`. Re-issue the same `cleaners { team { name } }` request from step 4 → `expect(getTeamsByIdsSpy).toHaveBeenCalledTimes(1)` **and** `expect(new Set(getTeamsByIdsSpy.mock.calls[0][0])).toEqual(new Set(expectedTeamIds))` (exact-set equality, not `arrayContaining`, since duplicate or partial argument sets would otherwise satisfy a looser check). Re-issue the same `teams { cleaners { id } }` request → the equivalent two-part assertion against `listCleanersByTeamIds`'s spy, `expectedTeamIds` there including Team C's id (an empty-result key must still appear in the batch call's argument set). This is the test that actually fails if a resolver regresses to one `getTeam`/`listTeamCleaners` call per parent row — step 4 alone would still pass in that regression, since both a batched and an N+1 implementation return identical data.
  6. **Request-isolation proof**: issue two *separate* Supertest requests (not two field resolutions within one request) — both issuing the same `teams { id name cleaners { id } }` query, so both resolve `teamCleanersLoader` against the SAME cache keys (the same fixture team ids from step 4), not a freshly-seen key. This is the crux of the proof: a `DataLoader`'s cache is keyed per-value, so querying a brand-new key is always a cache miss and always hits the database regardless of whether the loader instance is request-scoped or a stale singleton — that would prove nothing. Between the two requests, reassign a cleaner from Team A to Team B via `assignCleanerToTeam`. The second request's result must reflect the reassignment, not the first request's stale membership — proving the request-scoped `CleanerTeamLoaders` provider is actually constructed fresh per request rather than reused/singleton (§3). A singleton-scoped loader would still pass steps 1–5 (which each issue only one relevant request) but would fail this step: its `teamCleanersLoader` would still have those team ids cached from the first request and would return the stale, pre-reassignment membership instead.
...
| New dependency flagged for explicit approval before use | ✅ — §3/§4, `dataloader`; approved at M5 |
