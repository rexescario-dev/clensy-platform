...
| **Revision note** | M5 round 1 (reviewer: project owner) requested 2 blockers + 3 recommendations. Verified against the actual current repository files (not a cached copy) before acting: the two flagged blockers — stale `priceCents`/USD language in the Accepted specification, and a missing `updateAddOn`/`add_on.update` row in this plan's audit contract — were not present in either live document; both already read exactly as the reviewer's own "required" state (§4.7/§4.1 of the spec use `priceMinorUnits` throughout with no live `priceCents` field definition, and this plan's §2/§4.4/Task-2-tests/Task-7 already list `updateAddOn`/`add_on.update` as the fifth mutation everywhere). No content change was made for either — re-flagging this here so the ledger reflects that the check ran, not just the reviewer's claim. The three recommendations were genuine and applied: `AddOnsService.getAddOn()` changed from "add only if Task 4 needs it" to a definitive decision not to add it, with `updateAddOn`'s existence check going straight through the transaction manager (Task 2). The N+1 query-count invariant in §7 was restated as an explicit named assertion — exactly one `getActivePricingForServiceIds` call per request, with the complete N-service-id set as its argument — rather than leaving that precision implicit in Task 7's prose alone. The UI's peso-to-minor-units conversion (Task 6, `format-price.ts` and the service/add-on detail pages) was rewritten from floating-point `× 100`/`÷ 100` arithmetic to a deterministic string-parsing/integer-arithmetic implementation (`parsePesosToMinorUnits`, `formatMinorUnits` via `Math.trunc`/`%`, never `parseFloat`/`.toFixed()` on a divided float). |
| **M5 decision** | **Accepted** — 2026-08-16. Three genuine recommendations applied (`getAddOn()` removed definitively, N+1 invariant stated explicitly, deterministic money parsing); the two blockers were checked against the live repository and found already satisfied. No remaining blockers. Ready for M6 Implementation. |
...
3. **Integration/e2e** (Jest + Supertest against a real Postgres via `AppModule`, mirroring `cleaners-teams.e2e-spec.ts`): the full GraphQL-request-to-database golden path in spec §2 (create service → attach pricing → list active catalog), RBAC-allowed/denied cases per the §4.3 matrix, and — distinct from the functional correctness above — a **query-count** proof of the `Service.activePricing` N+1 batching invariant (§3's `ActivePricingLoader` mechanism), because functional correctness alone cannot distinguish a correctly batched implementation from an N+1 one that happens to return the right data. **The exact invariant under test (stated explicitly, M5 round 1): for a `services` response containing N services, `PricingRulesService.getActivePricingForServiceIds` is called exactly once per GraphQL request, with an argument set equal to the complete set of those N services' ids** — not merely "called," and not merely "called once with *some* ids," since either weaker assertion would still pass under a regression that calls the bulk method once per row with a single-element argument list. That test `jest.spyOn`s the real `PricingRulesService.getActivePricingForServiceIds` instance resolved from the running `AppModule` (`app.get(PricingRulesService)`) and asserts both `toHaveBeenCalledTimes(1)` and exact-set equality between its single call's argument and the expected id set. See Task 7.
...
- `apps/api/src/modules/catalog/application/services/add-ons.service.ts` — `AddOnsService`, structurally identical to `ServicesService` (Task 1), substituting `AddOn`'s own fields:
  ...
  - **`AddOnsService` does not expose a `getAddOn(id)` method** (decided definitively, M5 round 1 — spec §4.5 lists no `addOn(id)` GraphQL query, and no other caller in this plan needs a single-`AddOn` read path): `updateAddOn`'s existence check goes directly through the transaction manager, `manager.findOneBy(AddOnEntity, { id })`, the same technique `updateService`/`updateCleaner` already use for their own entities. No read method with zero callers is added to this service.
...
- `apps/web/app/catalog/format-price.ts` — two client-side money helpers, both deterministic and neither ever touching floating-point arithmetic (M5 round 1 — the initial draft's `minorUnits × 100`/`÷ 100` used `number` multiplication/division for currency, which is unnecessary floating-point risk for a value this codebase otherwise keeps as an exact integer everywhere else):
  ```ts
  ...
  ```
  `formatMinorUnits` uses integer `Math.trunc`/`%` (never `.toFixed()` on a divided float, which is the exact pattern that risks a rounding artifact). `parsePesosToMinorUnits` never calls `parseFloat`/multiplies by `100` — it validates the input as a whole-and-up-to-two-fractional-digits decimal string via regex, then builds the integer minor-units value by string concatenation and a single `parseInt`, so no intermediate floating-point value ever exists. Both are the one client-side money-handling surface spec §4.5/§2 assign to `apps/web`; not a `packages/ui` primitive (§4 — no new shared UI primitives), just plain functions shared by the pages below.
