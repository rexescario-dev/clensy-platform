...
| **Revision note** | Three prior M5 rounds resolved a circular contract dependency, a leaked `EntityManager` parameter, wrong module-import direction, cross-origin cookie mechanics, a last-active-Owner race condition, and a missing email-uniqueness constraint. A fourth pass tightened remaining implementation-level ambiguity without further design changes: `AuditLogger`'s context-sensitive best-effort/transactional behavior stated explicitly (the single public method's two behaviors were previously described inconsistently across Task 1 and Task 3); the Owner-lock query corrected to lock the matching rows rather than an aggregate, plus a real-connection concurrency test; an explicit dependency-direction diagram; cookie `Path=/` and a single shared `JWT_EXPIRES_IN`-derived lifetime instead of two independently-maintained durations; the e2e's disabled-session check reframed around reusing the actual cookie, not abstract "JWT/cookie" mechanics; the `/admin` page's invalid-session handling (middleware never detects it; the page does, via `currentAdmin`) made explicit; and a JWT payload-shape regression test. |
| **M5 decision** | **Accepted** — 2026-08-14. No remaining blockers. Ready for M6 Implementation. |
...
- Session cookie name: a single named constant (e.g. exported from `platform/auth`'s config), read by both the `login` resolver's cookie-setting code and the JWT strategy's cookie extractor — never two independently-typed string literals that could drift apart. The cookie is set with `Path=/` (explicit, not left to the default) and `Max-Age` derived from the same `JWT_EXPIRES_IN` configuration value the JWT itself uses — one configured duration, not two independently-maintained 8-hour values.
- `Secure` cookie in local development: the session cookie is always emitted with `Secure=true` — no exception. Local development runs on `localhost`, where this works correctly without extra setup; if a non-`localhost` HTTP origin is ever required, HTTPS must be provisioned for it rather than weakening the cookie flag.
...
**Dependency direction, made explicit:**

```text
modules/admins  --implements-->  platform/auth's AdminIdentityLookupPort
modules/admins  --calls-------->  platform/audit's AuditLogger
platform/auth   --depends on--->  nothing under modules/*
AppModule (composition root)  --binds-->  ADMIN_IDENTITY_LOOKUP token to AdminIdentityLookupService
```

`platform/auth` never imports anything under `modules/*`; `modules/admins` depends on both platforms it needs (`platform/auth` for `Role`/the port it implements, `platform/audit` for logging) — that's the ordinary "module depends on platform" direction, not the inverted one §5.2 forbids. Only the identity-lookup binding needs the composition root's involvement, because that specific edge runs against the grain (an interface owned by `platform/auth`, implemented by `modules/admins`).
...
- `apps/api/src/platform/audit/infrastructure/audit-logger.service.ts` — implements the port. `log()` is the sole public method and is **context-sensitive**: outside an ambient state-change transaction (the login-event case), persistence failure is caught, logged via Nest's `Logger`, and never rethrown (best-effort); inside the transaction context established by `AdminsService` (Task 3), persistence uses that transaction's `EntityManager` and a persistence failure propagates so the surrounding transaction rolls back. Both behaviors live behind the one `log(event): Promise<void>` signature — callers never choose which mode; the implementation detects ambient context itself. The exact detection mechanism (e.g. a request/async-local-storage-scoped current-transaction service the implementation checks internally) is an M6 choice and MUST NOT appear in the public contract.
...
- `log()` outside an ambient transaction: given a repository mock that throws on save, `log()` resolves (does not throw) and the error reaches the injected `Logger` mock — proves the best-effort path.
...
- `log()` inside an ambient transaction: the context-sensitive path is exercised and verified end-to-end by Task 3's `admins.service.spec.ts` forced-audit-failure rollback assertions, not duplicated here — this task's tests cover only the standalone (non-transactional) implementation.
...
- `disable` concurrency (separate test, real database connections — not mocked repositories, since proving the lock requires proving actual Postgres locking behavior, which a mock cannot do): with exactly two active Owners, fire two concurrent `disable` calls targeting each other from two separate `EntityManager`/connection instances — at most one MUST succeed, the other MUST be rejected with the last-active-Owner error, and exactly one active Owner MUST remain afterward.
...
- `TokenService`: an issued token, decoded, has payload keys exactly `sub`, `iat`, `exp` — no `role` and nothing else. This is a direct regression guard against the tempting-but-forbidden shortcut of signing `{ sub, role }` (§4.1's no-role-claim rule).
...
- `apps/web/app/admin/page.tsx` — reads `currentAdmin` first. If the query fails or resolves with no authenticated principal (invalid/expired/disabled session — middleware only checked cookie presence, not validity), redirect to `/login`; this is where an invalid session actually gets caught, not in middleware. Otherwise, if `role !== OWNER`, renders a "not authorized" message instead of the management UI (a presentation nicety using `currentAdmin.role` for UX — API authorization per §4.2/§4.5 remains authoritative regardless of what this page renders, per §3). For an Owner: staff list (`admins` query, `DataTable` + `StatusBadge`), inline create-staff section (`createAdmin` mutation, `FormField`s for email/password/role), disable action per row (`disableAdmin` mutation)
- `apps/web/middleware.ts` — checks only for the session cookie's presence to redirect to `/login` when absent (§4.8's UX-hint-only contract — it does not decode the JWT or make any role decision). Expected behavior chain, stated explicitly so an implementer doesn't turn `middleware.ts` into an authentication endpoint: middleware checks cookie presence only → an expired/invalid/disabled-account session still passes middleware and reaches the page → the page's `currentAdmin` query is what actually discovers an invalid session (a GraphQL error/null result) → the page redirects to `/login` at that point. Middleware is never the layer that detects invalidity.
...
- `apps/api/test/admin-foundation.e2e-spec.ts` — implements spec §4.10 exactly, using the fixture helper in `beforeAll`/`beforeEach`:
  ...
  5. Preserve the Scheduler's session cookie exactly as returned by its `login` response (step 2); after Owner disables that Scheduler (step 3), issue a protected GraphQL request reusing that same cookie and assert it is rejected — exercising the full chain (cookie → JWT extraction → signature validation → fresh DB lookup → disabled account → rejection), not just JWT mechanics in isolation.
